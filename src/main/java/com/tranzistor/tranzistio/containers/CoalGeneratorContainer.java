...
public class CoalGeneratorContainer extends Container{
	...
	public final CoalGeneratorTileEntity te;
	private final IWorldPosCallable canInteractWithCallable;
	
	public CoalGeneratorContainer(final int windowId, final PlayerInventory playerInventory, final CoalGeneratorTileEntity te) {
		...
		this.addSlot(new Slot((IInventory) te, 0, 59, 35) {
			...
			public boolean mayPlace(ItemStack stack) {
				return CoalGeneratorTileEntity.getBurnTime(stack) > 0;
			}
		});
	...
	private static CoalGeneratorTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
		...
		if(te instanceof CoalGeneratorTileEntity) {
			return (CoalGeneratorTileEntity) te;
		}
	...
	public boolean stillValid(PlayerEntity player) {
		return stillValid(canInteractWithCallable, player, BlockInit.COAL_GENERATOR.get());
	}
	...
	public ItemStack quickMoveStack(PlayerEntity player, int index) {
		...
		if(slot != null && slot.hasItem()) {
			...
			stack = stack1.copy();
			int innerSlots = CoalGeneratorTileEntity.slots;
